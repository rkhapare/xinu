...
#include "spi.h"	/* Header file for SPI interface of BCM2835 */
/*-------------------------------------------------------------------------*/
/* Platform dependent macros and functions needed to be modified           */
/*-------------------------------------------------------------------------*/
#include <integer.h>
#include "gpio.h"						/* Include device specific declaration file here */

#define		INIT_PORT()	bcm2835_init()/*init_port()*/		/* Initialize MMC control port (CS=H, CLK=L, DI=H, DO=in) */
#define 	DLY_US(n)	bcm2835_delayMicroseconds(n)		/* Delay n microseconds */

#define		CS_H()		bcm2835_gpio_set(BCM2835_GPIO(4))	/* Set MMC CS "high" */
#define 	CS_L()		bcm2835_gpio_clr(BCM2835_GPIO(4))	/* Set MMC CS "low" */
#define 	CK_H()		bcm2835_gpio_set(BCM2835_GPIO(18))	/* Set MMC SCLK "high" */
#define		CK_L()		bcm2835_gpio_clr(BCM2835_GPIO(18))	/* Set MMC SCLK "low" */
#define 	DI_H()		bcm2835_gpio_set(BCM2835_GPIO(27))	/* Set MMC DI "high" */
#define 	DI_L()		bcm2835_gpio_clr(BCM2835_GPIO(27))	/* Set MMC DI "low" */
#define 	DO		bcm2835_gpio_lev(BCM2835_GPIO(17))	/* Test for MMC DO ('H':true, 'L':false) */
...
)
{
	...
	if (cmd == CMD12) spi_txrx(NULL, &d, 1);
	...
	if (count == 1) {
		...
		if (send_cmd(CMD18, sector) == 0) {	/* READ_MULTIPLE_BLOCK */
			do {
				...
				//if (!spi_txrx(NULL, buff, 512)) break;
				buff += 512;
			} while (--count);
	...
	if (count == 1) {				/* Single block write */
		...
		if (send_cmd(CMD25, sector) == 0) {	/* WRITE_MULTIPLE_BLOCK */
			do {
				...
				//if (!spi_txrx(buff, NULL, 0xFC)) break;
				buff += 512;
			} while (--count);
			if (!xmit_datablock(0, 0xFD)){	/* STOP_TRAN token */
			//if (!spi_txrx(0, NULL, 0xFD))
				count = 1;
			}
		}
	}
